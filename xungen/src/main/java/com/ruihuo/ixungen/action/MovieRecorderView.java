...
public class MovieRecorderView extends LinearLayout implements OnErrorListener {
    ...
    public MovieRecorderView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mWidth = a.getInteger(R.styleable.MovieRecorderView_record_width, DisplayUtilX.getDisplayWidth());//默认640
        mHeight = a.getInteger(R.styleable.MovieRecorderView_record_height, DisplayUtilX.getDisplayWidth());//默认360

        isOpenCamera = a.getBoolean(R.styleable.MovieRecorderView_is_open_camera, true);//默认打开摄像头
        recordMaxTime = a.getInteger(R.styleable.MovieRecorderView_record_max_time, 30);//默认最大拍摄时间为30s
...
//            LogUtil.e(LOG_TAG, recordFile.getAbsolutePath());
    ...
    private void initRecord() throws Exception {
        ...
        mediaRecorder.setAudioEncoder(AudioEncoder.AAC);//音频格式 AudioEncoder.AMR_NB这种音频格式无法在ios上面播放。
        ...
        if (sizePicture < 3000000) {//这里设置可以调整清晰度
            mediaRecorder.setVideoEncodingBitRate(3 * mHeight * mWidth);
        } else if (sizePicture <= 5000000) {
            mediaRecorder.setVideoEncodingBitRate(2 * mHeight * mWidth);
        } else {
            mediaRecorder.setVideoEncodingBitRate(1 * mHeight * mWidth);
        }
