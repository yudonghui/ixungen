...
import com.ruihuo.ixungen.ui.familytree.CallBackInterface;
...
public class HVScrollView extends FrameLayout {
    ...
    public void setBigSmall(CallBackInterface mCallBack) {
        this.mCallBack = mCallBack;
    }

    @Override
    public boolean onTouchEvent(MotionEvent ev) {
        int nCnt = ev.getPointerCount();//触摸的点数
        ...
        final int action = ev.getAction();
        if ((ev.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_POINTER_DOWN && 2 == nCnt)//<span style="color:#ff0000;">2表示两个手指</span>
        {
            ...
            int xlen = Math.abs((int) ev.getX(0) - (int) ev.getX(1));
            int ylen = Math.abs((int) ev.getY(0) - (int) ev.getY(1));

            nLenStart = Math.sqrt((double) xlen * xlen + (double) ylen * ylen);


        } else if ((ev.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_POINTER_UP && 2 == nCnt) {
            ...
            if (nLenEnd > nLenStart)//通过两个手指开始距离和结束距离，来判断放大缩小
            {
                ...
                mCallBack.callBack((int) nLenStart, (int) nLenEnd);
                Log.e("放大", nLenStart + "--" + nLenEnd);
            } else {
                mCallBack.callBack((int) nLenStart, (int) nLenEnd);
                Log.e("缩小", nLenStart + "--" + nLenEnd);
            }
        } else {
            switch (action & MotionEvent.ACTION_MASK) {
                case MotionEvent.ACTION_DOWN: {
    ...
    public void scaleViewAndChildren(View paramView, float paramFloat) {
        ...
        if ((localLayoutParams instanceof MarginLayoutParams)) {
            MarginLayoutParams localMarginLayoutParams = (MarginLayoutParams) localLayoutParams;
        ...
        if ((paramView instanceof TextView)) {
            ...
            localTextView.setTextSize(paramFloat * DisplayUtilX.Sp2Px(4));
        }
        ...
        if (localViewGroup != null) {
            Log.e("子元素的数量", "---" + localViewGroup.getChildCount());
            for (int i = 0; ; i++) {
