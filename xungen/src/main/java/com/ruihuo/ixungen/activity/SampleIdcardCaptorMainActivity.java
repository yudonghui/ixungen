...
//            actionBar.hide();
    ...
    public void onPreviewFrame(byte[] data, CameraManager.CameraProxy camera, int faces) {
        if(bIsAuto && mCaptor != null) {
            ...
            if (mCurrentFrameId < ApplicationParameters.CAMERA_DROP_FIRST_DARK_FRAME) {
                Log.i(TAG, "onPreviewFrame, drop frame id: " + mCurrentFrameId);
                return;
            }

            Log.i(TAG, "[BEGIN] onPreviewFrame, frame id: " + mCurrentFrameId);
            ...
            Log.i(TAG, "[END] onPreviewFrame, 当前帧处理是否处理成功: " + isFrameHandled);
        }
    }
    ...
    private void increaseClassObjectCount() {
        ...
        classObjectCount++;
        Log.e(TAG, "SampleIdcardCaptorMainActivity classObjectCount onCreate: " + classObjectCount);
    ...
    public void finalize() {
        try {
            ...
            Log.e(TAG, "无法完成finalize...", e);
        }
        // DEBUG: 检测是否有Activity泄漏。与increaseClassObjectCount对应
        classObjectCount--;
        Log.e(TAG, "SampleIdcardCaptorMainActivity classObjectCount finalize: " + classObjectCount);
    }

    // 手动拍摄到照片后的回调函数
    @Override
    public void onPictureTaken(byte[] data, CameraManager.CameraProxy camera) {
            Log.i(TAG, "[BEGIN] onPictureTaken...");

        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inPreferredConfig = Bitmap.Config.RGB_565;
        Bitmap bitmap = BitmapFactory.decodeByteArray(data, 0, data.length, options);

           Log.i(TAG, "original size " + bitmap.getWidth() + "x" + bitmap.getHeight());
        ...
        if (imageHeight != bitmap.getHeight() || imageWidth != bitmap.getWidth()) {
            bitmap = ImageUtil.getSubBitmap(bitmap, imageWidth, imageHeight);
                Log.d(TAG, "get sub image, width: " + bitmap.getWidth() + " imageHeight: " + bitmap.getHeight());
        }

            Log.i(TAG, "final image size, width: " + bitmap.getWidth() + " height: " + bitmap.getHeight());
        ...
        onIDCardCaptured(new CapturedIDCardImage(imageContent));
           ...
           Log.d(TAG, "[END] onPictureTaken done");
    }
