...
//    private boolean mIsDisplayTag;                                      // 是否显示标签
    ...
    protected Paint mSelectedDayBgPaint;               //选中日期的背景颜色
    protected Paint mSelectedDayStartEndPaint;              //选中入住和离店日期的背景颜色
    ...
    protected int mSelectedDayStartEndBgColor;                 // 选中的日期背景颜色
    ...
    public SimpleMonthView(Context context, TypedArray typedArray, DayPickerView.DataModel dataModel) {
        ...
        String date = CalendarUtils.longToDate(System.currentTimeMillis());
        String[] split = date.split("\\-");
        if (split != null && split.length == 3) {
            int year = Integer.parseInt(split[0]);
            int moth = Integer.parseInt(split[1]) - 1;
            int mothDay = Integer.parseInt(split[2]);
            time = new Time(year, moth, mothDay);
        } else time = new Time(2017, 7, 11);
...
//        mDayTextColor = typedArray.getColor(R.styleable.DayPickerView_colorDayName, resources.getColor(R.color.normal_day));
        ...
        mSelectedDayStartEndBgColor = typedArray.getColor(R.styleable.DayPickerView_colorSelectedDayStartEndBg, resources.getColor(R.color.selected_day_background));
...
//        mDrawRect = typedArray.getBoolean(R.styleable.DayPickerView_drawRoundRect, true);
    ...
    private String getMonthAndYearString() {
        ...
        Log.e("formatDateRange", s);
        return s;
    }

    private void onDayClick(SimpleMonthAdapter.CalendarDay calendarDay) {
        if (mOnDayClickListener != null && (isPrevDayEnabled || !prevDay(calendarDay.day, time))) {
            mOnDayClickListener.onDayClick(this, calendarDay);
        }
    }

    private boolean sameDay(int monthDay, Time time) {
        Log.e("m年月日", mYear + "月" + mMonth + "日" + monthDay);
        Log.e("m年月日", time.year + "月" + time.month + "日" + time.monthDay);
    ...
    private boolean prevDay(int monthDay, Time time) {
        int linshiMoth;
        if (mMonth == 0) linshiMoth = 12;
        else linshiMoth = mMonth;
    ...
    protected void drawMonthCell(Canvas canvas) {
        ...
        while (day <= mNumCells) {
            ...
            if (!isPrevDayEnabled && prevDay(day, time)) {
                isPrevDay = true;
            ...
            if (mStartDate != null && cellCalendar.equals(mStartDate) && !mStartDate.equals(mEndDate)) {
                isBeginDay = true;
                drawDayBg(canvas, x, y, mSelectedDayStartEndPaint);
...
//                    canvas.drawRoundRect(rectF, 10.0f, 10.0f, mBusyDayBgPaint);
    ...
    protected void initView() {
        ...
        //  mWeekTextPaint.setFakeBoldText(true);//是否是粗体
...
//        mTitleBGPaint.setStyle(Style.FILL);
        ...
        // 被选中的入住离店日期背景paint
        mSelectedDayStartEndPaint = new Paint();
        mSelectedDayStartEndPaint.setFakeBoldText(true);
        mSelectedDayStartEndPaint.setAntiAlias(true);
        mSelectedDayStartEndPaint.setColor(mSelectedDayStartEndBgColor);
        mSelectedDayStartEndPaint.setTextAlign(Align.CENTER);
        mSelectedDayStartEndPaint.setStyle(Style.FILL);
...
//        }
        ...
        mCalendar.set(Calendar.MONTH, mMonth);//calendar  月份是从0到11
    ...
    class Time {
        ...
        public Time(int year, int month, int mothDay) {
            this.year = year;
            this.month = month;
            this.monthDay = mothDay;
        }
    }
}