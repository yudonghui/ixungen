...
import com.ruihuo.ixungen.entity.CityBean;
...
public class ChooseCityUtil implements View.OnClickListener, NumberPicker.OnValueChangeListener {
    ...
    public void createDialog(Context context, String[] oldCityArray, ChooseCityInterface cityInterface) {
        ...
        Gson gson=new Gson();
        bean = gson.fromJson(ReaderFileUtils.ReadTxtFile(context,"location.json"),CityBean.class);
    ...
    private void changeCity(int provinceTag) {
      ...
      //  LogUtils.i(Arrays.toString(cityArray));
        try {
    ...
    private void setNomal() {
        ...
        //设置字体颜色
        setNumberPickerTextColor(npProvince, context.getResources().getColor(R.color.green_txt));
        setNumberPickerTextColor(npCity, context.getResources().getColor(R.color.green_txt));
        setNumberPickerTextColor(npCounty, context.getResources().getColor(R.color.green_txt));
    }
